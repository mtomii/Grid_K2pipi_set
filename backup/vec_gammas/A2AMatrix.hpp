...
private:
    ...
    typedef std::function<std::string(const unsigned int, const unsigned int, const unsigned int)>  FilenameMo;
...
public:
  ...
  //template <typename FImpl>
    ...
    void execute0(const std::vector<Field> &left, 
		  ...
		  const std::vector<Field> &loop1,
		  const std::vector<Field> &loop2,
		  const A2AMatrix<Complex> &meson,
		  const int &it_mes,
                  A2AKernel<T, FImpl, Field> &kernel,
                  const FilenameMo &ionameFn,
                  const FilenameMo &filenameFn,
                  const MetadataFn &metadataFn,
		  const int ngamma);
    void execute1(const std::vector<Field> &left, 
                  const std::vector<Field> &right,
    ...
    for(int j=0;j<N_j;j+=blockSize_)
    {
	  ...
	  LOG(Message) << "All-to-all matrix cashe block " 
	    ...
	    */
		...
		if ( s +iii+jjj == 0 ) std::cout << "TEST:: " << t << " " << mCacheBlock(e,s,t,iii,jjj) << std:: endl;
              }
            });
...
::execute0(const std::vector<Field> &left,  const std::vector<Field> &right,
	   const std::vector<Field> &loop1, const std::vector<Field> &loop2,
	   const A2AMatrix<Complex> &mes, const int &it_mes,
	   A2AKernel<T, FImpl, Field> &kernel, const FilenameMo &ionameFn,
	   const FilenameMo &filenameFn, const MetadataFn &metadataFn,
	   const int ngamma)
{
  //////////////////////////////////////////////////////////////////////////
  // i,j   is first  loop over blockSize_ factors
  ...
  for(int j=0;j<N_j;j+=blockSize_)
  {
    ...
    ioTime = -GET_TIMER("IO: write block");
    START_TIMER("IO: total");
    makeFileDir(filenameFn(0, 0, 0), grid_);
#ifdef HADRONS_A2AM_PARALLEL_IO
    grid_->Barrier();
    // make task list for current node
    nodeIo_.clear();
    for(int f = myRank; f < next_*nstr_; f += nRank)
    {
      ...
      h.e  = f/nstr_;
      h.s  = f % nstr_;
      h.io = A2AMatrixIo<TIo>(filenameFn(h.e, h.s, it_mes), 
			      ionameFn(h.e, h.s, it_mes), nt_, N_i, N_j);
      h.md = metadataFn(h.e, h.s);
      nodeIo_.push_back(h);
    }
    // parallel IO
