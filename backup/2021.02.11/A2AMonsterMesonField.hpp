...
public:
    GRID_SERIALIZABLE_CLASS_MEMBERS(A2AMonsterMesonFieldPar,
				    ...
				    int, delt_min,// signed max relative distance of 4-quark operator to meson field
                                    std::string, output,
                                    std::string, gammas1,
				    std::string, gammas2);
};
...
public:
  MonsterMesonFieldKernel(const std::vector<std::vector<Gamma::Algebra> > &gamma1,
			  ...
			  GridBase *grid)
      // t_mes_os_(t_mes_os),
    : gamma1_(gamma1), gamma2_(gamma2), nameg1_(nameg1), nameg2_(nameg2), type_(type), t_mes_os_(t_mes_os), delt_max_(delt_max), delt_min_(delt_min), grid_(grid)
    {
        vol_ = 1.;
...
void TA2AMonsterMesonField<FImpl>::execute(void)
{
    ...
    Kernel kernel(gamma1_, gamma2_, nameg1_, nameg2_, type_, t_mes_os_, delt_max_, delt_min_, envGetGrid(FermionField));

    auto ionameFn = [this](const unsigned int g, const unsigned int it)
    {
        std::stringstream ss;
    ...
    auto filenameFn = [this, &ionameFn](const unsigned int g, const unsigned int it)
    {
        return par().output + "." + std::to_string(vm().getTrajectory()) 
	  + "/" + ionameFn(g, it) + ".h5";
    };

    auto metadataFn = [this](const unsigned int g, const unsigned int it_meson)
    {
        A2AMonsterMesonFieldMetadata md;
	md.timeConfig.push_back(t_mes_os_[it_meson]);
	md.timeConfig.push_back(delt_max_);
	md.timeConfig.push_back(delt_min_);
    ...
    for( int it = 0; it < t_mes_os_.size(); ++it ) {
      ...
      std::cout << "# t_mes: " << t_mes_os_[it] << std::endl;
      for( int t = 0 ; t < nt ; ++t ) {
	int t_mes = t_mes_os_[it];
	int stepsize = delt_max_ - delt_min_;
	int v_os = t_mes_os_[it] + delt_min_;
	if ( stepsize > 0 )
	  t_mes = int( ((t - v_os + nt - 1)%nt)/stepsize ) * stepsize + t_mes_os_[it];
	//t_mes = ( int( (t + delt_max_ - t_mes_v) / delt_max_ ) * delt_max_ + t_mes_v ) % nt;
	if ( delt_max_ < 0 )
	  t_mes = ( int( (t - stepsize - v_os ) / (-stepsize) ) * (-stepsize) + t_mes_os_[it] ) % nt;
	  //t_mes = ( int( ((t - t_mes_v + nt - 1)%nt)/(-delt_max_)) * (-delt_max_) + t_mes_v ) %nt;
	...
	meson.push_back(meson_f[t_mes]);
      ...
      if ( type_ == 1 ) computation.execute1(left, right, loop1, loop2, meson, it, kernel, ionameFn, filenameFn, metadataFn);
