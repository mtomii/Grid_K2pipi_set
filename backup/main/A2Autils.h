...
// Omitting lhs_wi, rhs_vj, gammas, orthogdim, t_kernel, t_gsum
  ...
  thread_for(r,rd,{
	...
	for(int mu=0;mu<Ngamma;++mu){
	  SpinMatrix_v tmp2 = Gamma(gammas[mu])*tmp;
	  for(int s=0;s<Ns;++s){
	    cf[mu+Ngamma*ss]()()() += tmp2()(s,s)();
	  }
	  //= trace(Gamma(gammas[mu])*tmp) + cf[mu+Ngamma*ss]()()();
	}
	/*
	auto mom_v = mom[0].View();
	auto phase = mom_v[ss];
	mac(&mat[ss],&vv,&phase);
	*/
      }
...
// Omitting lhs_wi, rhs_vj, gammas, orthogdim, t_kernel, t_gsum
  ...
  thread_for(r,rd,{
	...
	for(int mu=0;mu<Ngamma;++mu){
	  SpinColourMatrix_v tmp2 = Gamma(gammas[mu])*tmp;
	  for(int c1=0;c1<Nc;c1++)
	  for(int c2=0;c2<Nc;c2++)
	  for(int s=0;s<Ns;++s){
	    cmat[mu+Ngamma*ss]()()(c1,c2) += tmp2()(s,s)(c1,c2);
	  }
	}
      }
    }}
  ...
  thread_for(r,rd,{
	...
	for(int j=0;j<Nj;j++){
	  ...
	  for(int i=0;i<Ni;i++){
	    auto lhs_v = v[i].View();
	    auto left  = lhs_v[ss];
	    s1c1j()()() += left()(s1)(c1) * Pi(i_in+i,j_in+j);
	  }
	  auto rhs_v = w[j].View();
	  auto right = conjugate(rhs_v[ss]);
      ...
      for(int mu=0;mu<Ngamma;mu++) {
	Scalar_v scl; Trace(scl,vv,Gamma(gammas[mu]));
	cf[mu+Ngamma*ss]()()() += scl()()();
      }
    }}
  });
...
// Omitting lhs_wi, rhs_vj, gammas, orthogdim, t_kernel, t_gsum
  ...
  thread_for(r,rd,{
	...
	for(int j=0;j<Nj;j++){
      ...
      for(int mu=0;mu<Ngamma;++mu){
	SpinColourMatrix_v tmp2 = Gamma(gammas[mu])*vv;
	for(int c1=0;c1<Nc;c1++)
	for(int c2=0;c2<Nc;c2++)
	for(int s=0;s<Ns;++s){
	    cmat[mu+Ngamma*ss]()()(c1,c2) += tmp2()(s,s)(c1,c2);
			       ...
			       const int Nblock, const int offset)
{
  ...
  thread_for(r,rd,{
    ...
    for(int b=0;b<e2;b++){
      int ss= so+n*stride+b;
      for(int mu=0;mu<Ngamma;++mu){
	SpinColourMatrix_v WM = Gamma(gammas1[mu]) * loop_m[ss] * Gamma(gammas2[mu]);
	for(int i=0;i<Nblock;i++){
	//SpinColourMatrix_v vv = Zero();
	  ...
	  left_loop(mu,i+offset,ss) = left_v;
	}
      }
    }}
  });
			       ...
			       const int Nblock, const int offset)
{
  ...
  thread_for(r,rd,{
    ...
    for(int b=0;b<e2;b++){
      int ss= so+n*stride+b;
      for(int mu=0;mu<Ngamma;++mu){
	ColourMatrix_v CM = loop_m[mu+Ngamma*ss];
	for(int i=0;i<Nblock;i++){
	//SpinColourMatrix_v vv = Zero();
	  auto lhs_v = left_v[i].View();
	  auto left = conjugate(lhs_v[ss]);
	  ...
	  left_loop(mu,i+offset,ss) = left_v * Gamma(gammas1[mu]);
	}
      }
    }}
  });
			       ...
			       const int Nblock, const int offset)
{
  ...
  thread_for(r,rd,{
    ...
    for(int b=0;b<e2;b++){
      int ss= so+n*stride+b;
      for(int mu=0;mu<Ngamma;++mu){
	SpinMatrix_v WM = Gamma(gammas1[mu]) * loop_m[ss] * Gamma(gammas2[mu]);
	for(int i=0;i<Nblock;i++){
	//SpinColourMatrix_v vv = Zero();
	  auto lhs_v = left_v[i].View();
	  auto left = conjugate(lhs_v[ss]);
  ...
  thread_for(r,rd,{
	...
	for(int i=0;i<Lblock;i++){
	  ...
	  for(int j=0;j<Rblock;j++){
	    ...
	    int base = Ngamma*(i+Lblock*(j+Rblock*r));
	    for(int mu=0;mu<Ngamma;++mu){
	      Scalar_v scl; Trace(scl,tmp,Gamma(gammas1[mu]));// v1 2s or less
		...
		= ( tmp2()(0,0)()+tmp2()(1,1)()
		   +tmp2()(2,2)()+tmp2()(3,3)() );
	      */
	      lvSum[mu+base]()()() += scl()()() * loop_s[mu+Ngamma*ss]()()();
	    }
	  }
	}
      }
