...
template<class vtype,int N>
accelerator_inline auto operator*(const iVector<vtype, N> &arg, const Gamma &G)
  ->typename std::enable_if<matchGridTensorIndex<iVector<vtype,N>,SpinorIndex>::notvalue,iVector<vtype,N>>::type 
{
  iVector<vtype,N> ret;
  for(int i=0;i<N;i++){
    ret._internal[i]=arg._internal[i]*G;
  }
  return ret;
}
...
accelerator_inline void Trace(iScalar<stype> &ret, const iScalar<mtype> &arg, const Gamma &G) 
{
  ret._internal=Trace(arg._internal,G);
}
