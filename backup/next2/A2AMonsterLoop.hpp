...
void TA2AMonsterLoop<FImpl>::execute(void)
{
  ...
  for( int tl = 0 ; tl < grid->_ldimensions[3] ; ++tl ) {
    int t = tl + grid->_lstart[3];
...
#if 0
    A2AMatrix<Scalar_s> mat0(N_i,N_j);
    for(int i=0;i<N_i;i++)
    for(int j=0;j<N_j;j++) {
      mat0(i,j)()()() = meson_f[t_mes](i,j);
    }
    meson.push_back(mat0);
#endif
  ...
  thread_for(r,rd,{
    ...
    for(int b=0;b<e2;b++){
      ...
      for(int j=0;j<N_j;j++){
	//Scalar_v s1c1j = Zero();
	SpinColourVector_v vec = Zero();
	auto rhs_v = loop2[j].View();
	auto right = conjugate(rhs_v[ss]);
	for(int i=0;i<N_i;++i){
	  auto lhs_v = loop1[i].View();
	  auto left  = lhs_v[ss];
	  for(int s1=0;s1<Ns;s1++)
	  for(int c1=0;c1<Nc;c1++){
	    vec()(s1)(c1) += left()(s1)(c1) * meson[r](i,j);
	...
	for(int c2=0;c2<Nc;c2++){
	  mat[ss]()(s1,s2)(c1,c2) += vec()(s1)(c1) * right()(s2)(c2);
	}
      }
    }}
  });
  ...
  for(int j=0;j<N_j;j+=par().block) {
    ...
    for(int jj=0;jj<N_jj;jj+=par().cacheBlock) {
      ...
      std::cout << "AAA " << meson.size() << std::endl;
