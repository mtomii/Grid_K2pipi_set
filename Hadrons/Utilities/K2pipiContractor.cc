...
using MatrixSet = Eigen::Tensor<Mat, 2>;
...
namespace Contractor
{
    ...
    public:
        GRID_SERIALIZABLE_CLASS_MEMBERS(ProductPar,
                                        ...
                                        std::vector<std::string>, lastMats,
                                        std::string, translations,
					std::string, fileStemTail,
					unsigned int, min_t,
					unsigned int, max_t,
...
int main(int argc, char* argv[])
{
  ...
  read(reader, "a2aMatrixLast", par.a2aMatrixLast);
  ...
  std::map<std::string, std::vector<A2AMatrixTr<ComplexD>> > a2aMatLast;
  //    unsigned int                                     cacheSize;
       ...
       traj < par.global.trajCounter.end; traj += par.global.trajCounter.step)
  {
    ...
    for ( unsigned int im = 0; im < par.a2aMatrixLast.size(); ++im )
    {
      TimerArray                             tAr;
      double                                 fusec, busec, flops, bytes;
      std::string name = par.a2aMatrixLast[im];
      double      t;
      //	    double  size;

      std::cout << "* Caching transposed last term " << name << std::endl;
	...
	const A2AMatrix<ComplexD> &ref = a2aMat.at(name)[t];
    ...
    for (auto &p: par.product)
    {
      ...
      //MatrixSet<A2AMatrixTr<ComplexD>>       lastTerm(p.lastMats.size(),par.global.nt);
      //std::vector<A2AMatrixTr<ComplexD>>     lastTerm(par.global.nt);
	...
	if (std::find(result.a2aMatrix.begin(), result.a2aMatrix.end(), m) == result.a2aMatrix.end())
	{
      ...
      std::string              fileStem = "", filename, dataSet;
      std::vector<std::string> terms = strToVec<std::string>(p.terms);

      for (unsigned int i = 0; i < terms.size(); i++)
      {
	if ( i != 0 ) fileStem += "_";
	fileStem += terms[i] + "_" + p.times[i];
	  //std::to_string(result.times[i]);
      }
      const std::string dir = par.global.output;
      std::string fileStem2 = fileStem + p.fileStemTail;
      filename = dir + "/" + RESULT_FILE_NAME(fileStem2, traj);
      std::cout << "Saving correlator to '" << filename << "'" << std::endl;
      makeFileDir(dir);
      ResultWriter writer(filename);
	...
	for (auto &dt: translations)
	{
	  ...
	  std::cout << std::setw(8) << "traces with "
		    << p.lastMats.size() << " A2A Fields";
	  flops  = 0.;
	  bytes  = 0.;
	  fusec  = tAr.getDTimer("tr(A*B)");
	  busec  = tAr.getDTimer("tr(A*B)");
	  for (unsigned int j = 0; j < p.lastMats.size(); ++j ) {
	    std::string name = p.lastMats[j];
	    for (unsigned int tLast = p.min_t+dt; tLast <= p.max_t+dt; ++tLast)
	    {
	      tAr.startTimer("tr(A*B)");
	      A2AContraction::accTrMul(result.correlator[TIME_MOD(tLast - dt)], prod, a2aMatLast[name][TIME_MOD(tLast)]);
	      tAr.stopTimer("tr(A*B)");
	      flops += A2AContraction::accTrMulFlops(prod, a2aMatLast[name][TIME_MOD(tLast)]);
	      bytes += 2.*prod.rows()*prod.cols()*sizeof(ComplexD);
	    }
	    if (!p.translationAverage)
	    {
	      dataSet = fileStem + "_" + name;
	      dataSet += "_dt_" + std::to_string(dt);
