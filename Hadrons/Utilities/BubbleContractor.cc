...
Source file: Hadrons/Utilities/BubbleContractor.cc
...
void saveCorrelator(const Contractor::CorrelatorResult &result,
...
int main(int argc, char* argv[])
{
       ...
       traj < par.global.trajCounter.end; traj += par.global.trajCounter.step)
  {
	...
	for (unsigned int dt = 0; dt < par.global.nt; ++dt)
	{
	  std::cout << " -- positions= " << t << ", dt= " << dt << std::endl;
	  if (term.size() > 2)
	  {
	  ...
	  bytes  = 0.;
	  tAr.startTimer("Disk vector overhead");
	  prod = a2aMat.at(term[0])[TIME_MOD(dt)];
	  tAr.stopTimer("Disk vector overhead");
	  if ( term.size() == 1 ) {
	    result.times.push_back(0);
	    assert ( prod.rows() == prod.cols() );
	    tAr.startTimer("Linear algebra");
	    for ( int a = 0; a < prod.rows(); ++a ) {
	      result.correlator[dt] += prod(a,a);
	    }
#if 0
	    thread_for(a,prod.rows(),
	      {
		result.correlator[dt] += prod(a,a);
	      });
#endif
	  ...
	  {
	    tAr.startTimer("Disk vector overhead");
	    const A2AMatrix<ComplexD> &ref = a2aMat.at(term[j])[TIME_MOD(t[j-1] + dt)];
	    tAr.stopTimer("Disk vector overhead");
	  ...
	  unsigned int tLast = TIME_MOD( t[term.size() - 2] + dt );
	  tAr.startTimer("tr(A*B)");
	  A2AContraction::accTrMul(result.correlator[dt], prod, lastTerm[tLast]);
	  ...
	  std::cout << t << ":  t0 = " << dt << ", tLast = " << tLast << std::endl;
	  dti++;
	}// dt
	saveCorrelator(result, par.global.output, fileStem, traj);
      }
      tAr.stopTimer("Total");
