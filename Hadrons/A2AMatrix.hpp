...
public:
    ...
    template <typename MetadataType>
    void initFile(const MetadataType &d, const unsigned int chunkSizeI, const unsigned int chunkSizeJ);
    // block I/O
...
template <typename TIo, typename MetadataType>
void saveA2AMatrixSet(const A2AMatrixSet<TIo> mf,
		      const std::string &ionameFn,
		      const std::string &filenameFn,
		      const MetadataType &metadataFn,
		      const int lsize, const int rsize, const int nt);
/******************************************************************************
...
template <typename T>
template <typename MetadataType>
void A2AMatrixIo<T>::initFile(const MetadataType &d, const unsigned int chunkSizeI, const unsigned int chunkSizeJ)
{
#ifdef HAVE_HDF5
    std::vector<hsize_t>    dim = {static_cast<hsize_t>(nt_),
                                   static_cast<hsize_t>(ni_),
      ...
      //chunk = {static_cast<hsize_t>(nt_),
                            chunk = {static_cast<hsize_t>(1),
                                     static_cast<hsize_t>(chunkSizeI),
                                     static_cast<hsize_t>(chunkSizeJ)};
    if ( ni_ < chunkSizeI ) chunk[1] = ni_;
    if ( nj_ < chunkSizeJ ) chunk[2] = nj_;
    H5NS::DataSpace         dataspace(dim.size(), dim.data());
    H5NS::DataSet           dataset;
    H5NS::DSetCreatPropList plist;
...
void saveA2AMatrixSet(const A2AMatrixSet<TIo> mf,
		      ...
		      const int lsize, const int rsize, const int nt)
{
  A2AMatrixIo<HADRONS_A2AM_IO_TYPE> io(filename,ioname,nt,lsize,rsize);
  io.initFile(metadata, lsize, rsize);
  io.saveBlock(mf,0,0,0,0);
}
