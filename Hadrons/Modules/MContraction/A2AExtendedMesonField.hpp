...
public:
  ExtendedMesonFieldKernel(const std::vector<std::vector<Gamma::Algebra> > &gamma1,
			   const std::vector<std::vector<Gamma::Algebra> > &gamma2,
			   const std::vector<std::string> &nameg1,
			   const std::vector<std::string> &nameg2,
			   const int &type,
			   GridBase *grid)
    : gamma1_(gamma1), gamma2_(gamma2), nameg1_(nameg1), nameg2_(nameg2), type_(type), grid_(grid)
    {
        vol_ = 1.;
  ...
  {
    assert( type_ == 0 );
    //std::cout << "Starting ExtendedMesonField" << std::endl;
  ...
  virtual void operator()(FieldMatrix<SpinColourVector_v > &left_loop,
    ...
    {
      assert( type_ == 1 );
      A2Autils<FImpl>::LeftLoop1(left_loop, loop_m, left, gamma1_, gamma2_, Nblock, offset);
    }
  virtual void operator()(FieldMatrix<SpinColourVector_v > &left_loop,
			  const std::vector<ColourMatrix_v > &loop_m,
			  const FermionField *left, const int Nblock,
...
void TA2AExtendedMesonField<FImpl>::execute(void)
{
    ...
    auto ionameFn = [this](const unsigned int g)
    {
        ...
        ss << "type" << type_ << "_" << nameg1_[g] << "_" << nameg2_[g];
        return ss.str();
    };
    ...
    Kernel      kernel(gamma1_, gamma2_, nameg1_, nameg2_, type_, envGetGrid(FermionField));
